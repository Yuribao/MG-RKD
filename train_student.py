...
def get_args():
    ...
    parser.add_argument(
        "--teacher_num_layers", type=int, default=5, help="Teacher model number of layers"
    )
    parser.add_argument(
        "--norm_type", type=str, default="none", help="One of [none, batch, layer, pair]"
    )
    ...
    #     type=float,
    #     default=5.0,
    #     help="Temperature for softmax in knowledge distillation",
    # )
    parser.add_argument(
...
def run(args, num_layers, hidden_dim, dropout_ratio, temperature, lamb, weight, lamb2, seed):
    """
    Returns:
    ...
    teacher_names = "_".join(args.teacher)
    ...
    if args.exp_setting == "tran":
        output_dir = Path.cwd().joinpath(
            ...
            "transductive",
            args.dataset,
            f"{teacher_names}_{args.student}",
            f"{args.teacher_num_layers}",
            f"seed_{seed}",
        )
    ...
    g, labels, idx_train, idx_val, idx_test = load_data(
        ...
        ratio_train=args.ratio_train,#训练集和验证集标签分布
    ...
    feats = g.ndata["feat"].to(device) #从图对象 g 中提取节点的特征矩阵
    args.feat_dim = g.ndata["feat"].shape[1] #特征矩阵的列数，即每个节点的特征维
    args.label_dim = labels.int().max().item() + 1 #计算标签的类别数量
    ...
    emb_t = np.load(out_t_dir.joinpath("emb.npz"))["arr_0"]
    emb_t = torch.tensor(emb_t).to(device)
    teacher_train_acc = evaluator(out_t[idx_train], labels[idx_train])
    teacher_val_acc = evaluator(out_t[idx_val], labels[idx_val])
    teacher_test_acc = evaluator(out_t[idx_test], labels[idx_test])
    ...
    loss_and_score = []
    if args.exp_setting == "tran":
        idx_l = idx_train #硬标签 label
        idx_t = torch.cat([idx_train, idx_val, idx_test])#total
        ...
        obs_idx_train, obs_idx_val, obs_idx_test, idx_obs, idx_test_ind = graph_split(
            idx_train, idx_val, idx_test, args.split_rate, seed
        )
    ...
    logger.info(
        f"num_layers: {conf['num_layers']}. hidden_dim: {conf['hidden_dim']}. dropout_ratio: {conf['dropout_ratio']}. lamb: {conf['lamb']}. lamb2: {conf['lamb2']}. weight: {conf['weight']}. seed: {conf['seed']}"
    )
    logger.info(
    f"Teacher model accuracy  Val: {teacher_val_acc:.4f}, Test: {teacher_test_acc:.4f}"
    )
    ...
    return score_test, score_lst#score_lst
...
def main(num_layers, hidden_dim, dropout_ratio, temperature, lamb, weight, lamb2 ,seed):
    args = get_args()
    if args.num_exp == 1:
        score, score_lst = run(args, num_layers, hidden_dim, dropout_ratio, temperature, lamb, weight, lamb2, seed)
...
D={"num_layers":16, "hidden_dim":64, "dropout_ratio":0.3, "temperature":2, "lamb":0.5, "weight":0.5,"lamb2":0.5, "seed":5}
